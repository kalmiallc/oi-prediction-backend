...
async function requestAttestation(uid) {
  ...
  const rawAttestationRequest = {
    ...
    requestBody: {
      ...
      date:  (Math.floor(Number(event.startTime) - (Number(event.startTime) % 86400))).toString(),
...
async function setup() {
  ...
  const ATTESTATION_PROOF_ENDPOINT = 'https://attestation-coston.aflabs.net/attestation-client/api/proof/get-specific-proof'; // TODO: Move to env.
