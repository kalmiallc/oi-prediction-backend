...
async function requestAttestation(uid) {
  ...
  const VERIFICATION_ENDPOINT = `${ATTESTATION_PROVIDER_URL}/MatchResult/prepareRequest`;
...
async function getAttestationResult(uid) {
  ...
  const isValid = ["1", "2", "3"].includes(fullProof?.data?.responseBody?.result || '') && fullProof?.data?.responseBody?.timestamp;
  ...
  if (isValid) {
    ...
    if (isVerified) {
      ...
      event.winner = proof?.data?.response?.responseBody?.result || null;
      await event.save();
    } else {
