...
export function getInitialBets(sport, team1, team2, canBeTied) {
  ...
  if (canBeTied) {
    ...
    if (initialProb < 0.1) {
      ...
      const lowerProb = (initialProb - probabilities.probability_draw) / 2;
      probabilities.probability_a_wins -= lowerProb;
      probabilities.probability_b_wins -= lowerProb;
    }

    res['DRAW'] = initialProb * pool;
  }
  
  if (team1 === probabilities.team_a) {
    res[team1] = probabilities.probability_a_wins * pool;
    res[team2] = probabilities.probability_b_wins * pool;
  } else {
    res[team1] = probabilities.probability_b_wins * pool;
    res[team2] = probabilities.probability_a_wins * pool;
  }
